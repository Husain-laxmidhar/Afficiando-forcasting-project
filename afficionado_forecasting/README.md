...
## 1. Set up the conda environment (local development only)
...
If you'd rather use plain pip (e.g. inside an existing conda env), or if you're
...
> **Why `environment.yml` lives in `conda/` instead of the project root:**
> Streamlit Community Cloud auto-detects dependency files, and if it finds
> both `environment.yml` and `requirements.txt` in the same folder as
> `app.py`, it prefers conda -- even though the conda solve can fail on
> exact-version pins that resolve fine via pip (this happened with
> `plotly=5.20`, causing the whole dependency install to silently fall back
> to "just Streamlit", which then threw `ModuleNotFoundError: No module
> named 'plotly'`). Keeping `environment.yml` out of that folder guarantees
> Cloud always uses `requirements.txt` via pip/uv.

> Prophet and pmdarima can be slow to build on some platforms. If `conda env
> create` fails on those two, comment them out of `conda/environment.yml`,
> create the env, then install them individually:
...
- **Do not** put `environment.yml` in that same folder -- keep it under
  `conda/` as shipped here. If Cloud finds both a conda file and a pip file
  side by side, it uses conda, and a conda solve failure (e.g. on an
  exact-version pin like `plotly=5.20` that conda-forge won't resolve but
  pip resolves fine) causes it to silently install *only* bare Streamlit,
  producing `ModuleNotFoundError` for whatever you imported first.
...
- If you still hit a `ModuleNotFoundError` after this fix, open **Manage
  app -> logs** in Streamlit Cloud and check the dependency-install section
  near the top -- it will say exactly which file (conda or pip) was used
  and name any package that failed.
